const API = (page, limit) => fetch(
  `https://avl-frontend-exam.herokuapp.com/api/users/all?page=${page}&pageSize=${limit}`,
  {
    method: 'GET',
    headers: {
...
const handleErrors = (dispatch, error) => dispatch({
  type: 'PROFILE_LIST_FOLLOWER_FAILURE',
  error,
})

export default (page, limit) => async (dispatch) => {
  try {
    dispatch({ type: 'PROFILE_LIST_FOLLOWER_BEGIN' })

    const result = await API(page, limit)

    dispatch({
      type: 'PROFILE_LIST_FOLLOWER_SUCCESS',
      payload: result.data || [],
